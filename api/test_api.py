...
import logging
# from final_project.api.unitest import TestStringMethods
logging.basicConfig(level=logging.INFO)
mylogger = logging.getLogger()

userId= "cafecef9-475a-474b-8db4-995dd369f172"
...
# @pytest.fixture()
# def author() -> AuthorsDto:
#     book = BookDto("The Adventures of Tom Sawyer", "The Adventures of Tom Sawyer is an 1876 novel by Mark Twain about a boy growing up along the Mississippi River.",
#           50, 1, "https://mpd-biblio-covers.imgix.net/9781429959278.jpg", 6, 6)
#     author = AuthorsDto("George Orwell", 26.643, 84.91426, [book])
#     return author


def new_book(author_id) -> CreateBookDto:
    """
    A function that return details of new book
    :param author_id: id of the book's author
    """
...
@pytest.fixture()
def new_author() -> CreateAuthorDto:
    """
    A function that return login details of new author
    """
    new_author = CreateAuthorDto("Omer", 12.3, 12.4)
    return new_author


def all_authors_id(res) -> list:
    """
    A function that return all store's authors ids
    """
    authors_id = []
...
def all_books_id(res) -> list:
    """
    A function that return all store's books ids
    """
    books_id = []
...
def test_post_register(new_register, api_account):
...
def test_put_author_id(api_author, new_author):
    ...
    update_name = "UpdateNameAuthor"
    update_author = UpdateAuthorDto(update_name, float(res.json()[0]["homeLatitude"]), float(res.json()[0]["homeLongitude"]), author_id)
    res = api_author.put_author_byid(author_id, update_author)
    assert res.status_code == 204
    res2 = api_author.get_author_id(author_id)
    assert update_name == res2.json()["name"]
...
def test_put_author_id_invalid_id(api_author):
    ...
    while True:
        ...
        if num not in authors_id:
            update_name = "UpdateNameAuthor"
            update_author = UpdateAuthorDto(update_name, float(res_get_authors.json()[0]["homeLatitude"]),
...
def test_get_author_search(api_author):
    ...
    res_get_authors = api_author.get_authors()
    author_name = res_get_authors.json()[0]["name"]
    res_get_author_search = api_author.get_authors_search_txt(author_name)
    mylogger.info(res_get_author_search.text)
    for author in res_get_author_search.json():
...
def test_post_book(api_book, api_author):
    ...
    res_get_books = api_book.get_books()
    assert book.name in res_get_books.text
...
def test_post_book_empty_name(api_book, api_author):
    ...
    res_get_authors = api_author.get_authors()
    authors_id = res_get_authors.json()[0]["id"]
    newbook = new_book(authors_id)
    newbook.name = ""
    res_post_book = api_book.post_book(newbook)
    assert "The Name field is required." in res_post_book.text
    res_get_books = api_book.get_books()
    for book in res_get_books.json():
        assert newbook.name != book["name"]
...
def test_post_book_error_authorid(api_book):
    ...
    res_get_books = api_book.get_books()
    for book in res_get_books.json():
        assert newbook.authorId != book["authorId"]
...
def test_put_book_byid(api_book, api_author):
    ...
    res_get_books = api_book.get_books()
    update_name = "UpDateNameBook"
    book = BookDto(update_name, res_get_books.json()[0]["description"],
    ...
    res2 = api_book.get_book_byid(res_get_books.json()[0]["id"])
    assert update_name == res2.json()["name"]
...
def test_put_book_by_invalid_authorId(api_book, api_author):
    ...
    while True:
        ...
        if num not in authors_id:
            book = BookDto(res_get_books.json()[0]["name"], res_get_books.json()[0]["description"],
                               int(res_get_books.json()[0]["price"]), int(res_get_books.json()[0]["amountInStock"]),
                               res_get_books.json()[0]["imageUrl"], num, res_get_books.json()[0]["id"])
            res_put_book_id = api_book.put_book_byid(num, book)
            assert 400 == res_put_book_id.status_code
            break
...
def test_put_book_purchase_byid(api_book):
    ...
    res_get = api_book.get_books()
    for book in res_get.json():
        if int(book["amountInStock"]) > 0:
            book_amount = int(book["amountInStock"])
            res = api_book.put_book_purchase_byid(book["id"])
            assert book["name"] in res.text
            assert book_amount - 1 == res.json()["amountInStock"]
            break
